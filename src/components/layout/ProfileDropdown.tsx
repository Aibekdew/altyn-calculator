...
const ProfileDropdown: React.FC<Props> = ({
    ...
    const patchPwd = async () => {
        ...
        // после успешного изменения пароля сбрасываем форму и выходим из профиля
        setPwds({ old: "", neu: "" });
        setEditPwd(false);
        setSaving(false);
        alert("Пароль изменён. Войдите заново.");
        onLogout();            // прежний токен станет недействительным
    };
    ...
    return (
        <div className="relative">
            ...
            <button
                onClick={() => {
                    const willOpen = !open;          // что будет после клика
                    setOpen(willOpen);
                    if (willOpen) {
                        setEditFio(true);              // сразу раскрываем оба раздела
                        setEditPwd(true);
                    }
                }}
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -8 }}
                        className={`absolute ${mobile ? "left-0 w-full" : "right-0"
                    ...
                    >
                                ...
                                {editFio && (
                                    ...
                                    >
                                        ...
                                        <button
                                            onClick={patchFio}
                                            disabled={saving}
                                            className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Изменить
                                        </button>
                                    </motion.div>
                                ...
                                {editPwd && (
                                    ...
                                    >
                                        <input
                                            ...
                                            value={pwds.old}
                                            onChange={e => setPwds({ ...pwds, old: e.target.value })}
                                            className="w-full bg-white px-3 py-2 border rounded focus:ring"
                                        />
                                        <input
                                            type="password"
                                            ...
                                            value={pwds.neu}
                                            onChange={e => setPwds({ ...pwds, neu: e.target.value })}
                                            className="w-full bg-white px-3 py-2 border rounded focus:ring"
                                        />
                        ...
                        {/* --- печать (для mobile-варианта показываем здесь) --- */}
                        {mobile && onPrint && (
                            <button
                                onClick={onPrint}
                                disabled={!canPrint}
                  ...
                  ${canPrint
                        ...
                        <button
                            onClick={onLogout}
                            className="w-full mt-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-full flex items-center justify-center gap-2"
                        >
                            <LogOut size={16} />
                            Выйти
                        </button>
